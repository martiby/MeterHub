...
class Sml:
    ...
    def decode_frame(self, frame):
        ...
        if crc_calc == crc_frame:
            p = self.get_obis(frame, b'\x77\x07\x01\x00\x10\x07\x00\xff', 4, '>l')
            if p is None:
                p = self.get_obis(frame, b'\x77\x07\x01\x00\x0F\x07\x00\xff', 4, '>l')  # alternativ wegen EMH eHZ
            return {'e_import': self.get_obis(frame, b'\x77\x07\x01\x00\x01\x08\x00\xff', 8, '>q'),
